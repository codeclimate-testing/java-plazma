...
public abstract class AbstractExtCombo extends AbstractCombo {
	...
	getModel().addValue(index, value);
    }
    
    protected void doModelAdd(Object keyValue, Object displayValue) {
	getModel().addValue(keyValue, displayValue);
    }
    
    protected void doModelRemove(int index) {
	getModel().removeValue(index);
    }
    
    protected void doModelRemove(int start, int end) {
	for (int i = start; i <= end; i++) {
    	    getModel().removeValue(i);
    	}
    }
    
    protected void doModelRemoveAll() {
	getModel().removeAllValues();
    }
	...
	checkWidget();
    	if (displayValue == null) SWT.error(SWT.ERROR_NULL_ARGUMENT);
    	String string = getFormatText(displayValue); 
    	if (string == null) SWT.error(SWT.ERROR_NULL_ARGUMENT);
	doControlAdd(string);
	doModelAdd(keyValue, displayValue);
    }
    ...
    public String getNullTextValue() {
        return getModel().getNullTextValue();
    }



    public void setNullTextValue(String nullTextValue) {
	getModel().setNullTextValue(nullTextValue);
    }
    ...
    /**
     * Set data list. By default we reset current value
     */
    ...
    /**
     * Set data list. If resetValue is true then value will reset  
     * @param dataList
     * @param resetValue
     */
    public void setDataList(java.util.List dataList, boolean isResetValue) {
	
	removeAll(isResetValue);
	
	if (dataList == null) {
	    return;
	}
	for (int i = 0; i < dataList.size(); i++) {
	    addValue(dataList.get(i));
	}
    }

    public void setFlatDataList(java.util.List dataList) {
	removeAll();
	if (dataList == null) {
	    return;
	}
	for (int i = 0; i < dataList.size(); i++) {
	    ...
	    Object keyValue = getModel().getObjectKeyValue(value);
	    Object displayValue = getModel().getObjectDisplayValue(value);
	    
	    getModel().setRefValue(keyValue, value);
	    
	    addValue(keyValue, displayValue);
	}
    }
	...
	Object refValue = getModel().getRefValue(value);
	return refValue;
    }
