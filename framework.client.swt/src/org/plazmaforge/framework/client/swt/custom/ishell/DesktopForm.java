...
public class DesktopForm extends Composite
{
  ...
  public DesktopForm(Composite parent, int style)
  {
    ...
    setForeground(display.getSystemColor(brightness > 400 ? SWT.COLOR_BLACK : SWT.COLOR_WHITE));
    ...
    final Listener traverseFilter = new Listener()
    {
      public void handleEvent(Event event)
      {
        ...
        if(event.detail != SWT.TRAVERSE_TAB_NEXT && event.detail != SWT.TRAVERSE_TAB_PREVIOUS) return;
        if(!(event.widget instanceof Control)) return;
        Control c = (Control)event.widget;
        if(c.getShell() != shell) return;
        boolean[] desktopHit = new boolean[1];
        ...
        {
          if(event.detail == SWT.TRAVERSE_TAB_NEXT) activateNextShell();
  ...
  void manage(final InternalShell ishell)
  {
    ...
    if(ishell.isVisible()) visibleShells.add(ishell);
  ...
  void activate(InternalShell ishell)
  {
    ...
    if(ishell != null)
    {
      ...
      else
      {
        ...
        if(firstRegular != null && firstRegular != ishell) ishell.moveAbove(firstRegular);
        else
        {
          Control[] children = getChildren();
    ...
    activeShell = ishell;
    if(oldActiveShell != null) oldActiveShell.redrawDecorationsAfterActivityChange();
    if(ishell != null)
    {
  ...
  private InternalShell getTopmostRegularShell()
  {
    for(Control c : getChildren())
    {
      if(!(c instanceof InternalShell)) continue;
      if((c.getStyle() & SWT.ON_TOP) == 0) return (InternalShell)c;
    }
    return null;
  }
  ...
  void shellVisibilityChanged(InternalShell ishell, boolean visible)
  {
    if(visible)
    {
      if(!visibleShells.contains(ishell))
      {
        ...
        if(autoMaximize && !ishell.getMaximized() && (ishell.getStyle() & SWT.MAX) != 0 && hasVisibleMaximizedShell())
  ...
  private InternalShell getInternalShell(Control c, boolean[] desktopHit)
  {
    ...
    if(desktopHit != null && c == DesktopForm.this) desktopHit[0] = true;
    return null;
  }
  ...
  public void setAutoMaximize(boolean b)
  {
    ...
    if(hasMax)
    {
      ...
      for(InternalShell is : visibleShells)
      {
        if((is.getStyle() & SWT.MAX) != 0) is.setMaximized(true);
      }
    }
  }
  ...
  private void setAllVisibleMaximized(boolean maximized)
  {
    if(autoMaximize) // maximize or restore all shells
    {
      for(Control c : getChildren())
      {
        ...
        {
          InternalShell ishell = (InternalShell)c;
          if((ishell.getStyle() & SWT.MAX) != 0 && ishell.isVisible())
  ...
  private void activateNextShell()
  {
    ...
    if(visibleShells.size() < 2) return;
    ...
    if((activeShell.getStyle() & SWT.ON_TOP) != 0)
    {
      ...
      if(topReg != null) activate(topReg);
  ...
  private void activatePreviousShell()
  {
    ...
    InternalShell topReg = getTopmostRegularShell();
    InternalShell botTop = getBottommostOnTopShell();
    if(activeShell == topReg && botTop != null) activate(botTop);
    else
    {
      Control[] ch = getChildren();
      for(int i=ch.length-1; i>=0; i--)
  ...
  private void notifyDesktopListenersCreate(InternalShell ishell)
  {
    ...
    event.widget = ishell;
    for(DesktopListener l : desktopListeners) l.shellCreated(event);
  }
  ...
  private void notifyDesktopListenersDispose(InternalShell ishell)
  {
    Event event = new Event();
    event.widget = ishell;
    for(DesktopListener l : desktopListeners) l.shellDisposed(event);
  }
  ...
  private void notifyDesktopListenersActivate(InternalShell ishell)
  {
    Event event = new Event();
    event.widget = ishell;
    for(DesktopListener l : desktopListeners) l.shellActivated(event);
  }
  ...
  {
    for(InternalShell is : visibleShells)
      if(is.getMaximized()) return true;
    return false;
  }
