...
/** 
 * @author Oleh Hapon
 * $Id: UnitEditForm.java,v 1.2 2010/04/28 06:31:00 ohapon Exp $
 */

public class UnitEditForm extends AbstractEditForm {
     ...
     * @param style
     */
    public UnitEditForm(Composite parent, int style) {
	super(parent, style);
	initialize();
	...
	setTitle(Messages.getString("UnitEditForm.title")); //$NON-NLS-1$
	GridData gridData;
	...
	codeLabel.setText(Messages.getString("UnitEditForm.codeLabel.text")); //$NON-NLS-1$
	...
	nameLabel.setText(Messages.getString("UnitEditForm.nameLabel.text")); //$NON-NLS-1$
	...
	fullNameLabel.setText(Messages.getString("UnitEditForm.fullNameLabel.text")); //$NON-NLS-1$

    	fullNameField = new XTextField(this, SWT.BORDER);
    	gridData = new GridData(SWT.FILL, SWT.CENTER, true, false);
    	gridData.widthHint = 250;
    	fullNameField.setLayoutData(gridData);   
    	fullNameField.setTextLimit(25);
