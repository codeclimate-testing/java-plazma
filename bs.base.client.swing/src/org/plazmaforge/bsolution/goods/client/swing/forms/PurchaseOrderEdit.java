 ...
 * Created 31.05.2006
 */
public class PurchaseOrderEdit extends AbstractGoodsDocumentEdit implements IEnterValueHolder {
    ...
    private void initialize() {
        ...
        this.setEntityCode(PurchaseOrder.CLASS_ID);
    }
    ...
    protected TableModel createItemTableModel() throws ApplicationException {
        ...
        return createObjectTableModel(PurchaseOrderItem.class, columns);
    }



    private PurchaseOrder getPurchaseOrder() {
        return (PurchaseOrder) this.getEntity();
    }
    ...
    protected void updateView() throws ApplicationException {
        appendTitle(getPurchaseOrder().getWarehouseName());
        codeField.setText(getPurchaseOrder().getDocumentNo());
        dateField.setDate(getPurchaseOrder().getDocumentDate());
        partnerComboEdit.setValue(getPurchaseOrder().getPartner());
        warehouseComboEdit.setValue(getPurchaseOrder().getWarehouse());
        responsibleComboEdit.setValue(getPurchaseOrder().getMtrlResponsible());
        currencyComboBox.setValue(getPurchaseOrder().getCurrency());
        currencyRateField.setValue(getPurchaseOrder().getCurrencyRate());
        currencyRateDateField.setDate(getPurchaseOrder().getCurrencyRateDate());

        contractComboEdit.setValue(getPurchaseOrder().getContractDocument());
        reasonDocumentComboEdit.setValue(getPurchaseOrder().getReasonDocument());

        reasonField.setText(getPurchaseOrder().getReason());

        currencyTotalField.setValue(getPurchaseOrder().getCurrencyTotal());
        currencyTaxTotalField.setValue(getPurchaseOrder().getCurrencyTaxTotal());
        currencyTotalWithTaxField.setValue(getPurchaseOrder().getCurrencyTotalWithTax());        

        noteField.setText(getPurchaseOrder().getNote());
        AbstractStateTableModel productItemModel = (AbstractStateTableModel) productItemTable.getModel();
        productItemModel.addData(getPurchaseOrder().getProductItems());
    }

    private void updateAmount() {
        getPurchaseOrder().calculateGeneral();
        currencyTotalField.setValue(getPurchaseOrder().getCurrencyTotal());
        currencyTaxTotalField.setValue(getPurchaseOrder().getCurrencyTaxTotal());
        currencyTotalWithTaxField.setValue(getPurchaseOrder().getCurrencyTotalWithTax());        
    }
    ...
    protected void populateData() throws ApplicationException {
        getPurchaseOrder().setDocumentNo(codeField.getText());
        getPurchaseOrder().setDocumentDate(dateField.getDate());
        getPurchaseOrder().setWarehouse((Warehouse) warehouseComboEdit.getValue());
        getPurchaseOrder().setMtrlResponsible((MtrlResponsible) responsibleComboEdit.getValue());
        getPurchaseOrder().setPartner((PartnerHeader) partnerComboEdit.getValue());
        getPurchaseOrder().setCurrency((org.plazmaforge.bsolution.base.common.beans.Currency) currencyComboBox.getValue());
        getPurchaseOrder().setCurrencyRate(currencyRateField.doubleValue());
        getPurchaseOrder().setCurrencyRateDate(currencyRateDateField.getDate());

        getPurchaseOrder().setContractDocument((DocumentHeader) contractComboEdit.getValue());
        getPurchaseOrder().setReasonDocument((DocumentHeader) reasonDocumentComboEdit.getValue());
        getPurchaseOrder().setReason(reasonField.getText());

        getPurchaseOrder().setCurrencyTotal(currencyTotalField.doubleValue());
        getPurchaseOrder().setCurrencyTaxTotal(currencyTaxTotalField.doubleValue());
        getPurchaseOrder().setCurrencyTotalWithTax(currencyTotalWithTaxField.doubleValue());        

        getPurchaseOrder().setNote(noteField.getText());
        getPurchaseOrder().calculateGeneral();
    }
    ...
    protected class ProductItemType extends AbstractChildType {
        ...
        public EditForm getEditForm() throws ApplicationException {
            return getEDIT(PurchaseOrderItemEdit.class);
        }
    }
    ...
    protected void validateData() throws ApplicationException {
        ...
        validateRequiredField(buf, getPurchaseOrder().getDocumentDate(), dateLabel.getText());
        validateRequiredField(buf, getPurchaseOrder().getPartner(), partnerLabel.getText());
        validateRequiredField(buf, getPurchaseOrder().getWarehouse(), warehouseLabel.getText());
        checkError(buf);
    }
