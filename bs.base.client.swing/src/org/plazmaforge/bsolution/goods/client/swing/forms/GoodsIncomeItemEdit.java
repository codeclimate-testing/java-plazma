 ...
 * Date: 28.06.2004
 * Time: 15:44:48
 * $Id: GoodsIncomeItemEdit.java,v 1.4 2010/12/05 07:56:43 ohapon Exp $
 */
public class GoodsIncomeItemEdit extends AbstractGoodsDocumentItemEdit {

    
    public GoodsIncomeItemEdit() {
        super();
        initialize();
    }
    
    private void initialize() {
        this.setEntityClass(GoodsIncomeItem.class);
    }
	...
	return (GoodsIncome) this.getProductDocument();
    }

    private GoodsIncomeItem getGoodsIncomeItem() {
	return (GoodsIncomeItem) this.getEntity();
    }

    
    
    protected void updateView() throws ApplicationException {
        appendTitle(getGoodsIncomeItem().getProductName());
        goodsComboEdit.setValue(getGoodsIncomeItem().getProduct());
        quantityField.setValue(getGoodsIncomeItem().getQuantity());
        
        unitComboBox.initProduct(getGoodsIncomeItem().getProduct());
        unitComboBox.setSelectedUnit(getGoodsIncomeItem().getUnit());
        
        unitRateField.setValue(getGoodsIncomeItem().getUnitRate());
        currencyPriceField.setValue(getGoodsIncomeItem().getEnterCurrencyPrice());
        taxPercentField.setTaxId(getGoodsIncomeItem().getTaxId());
        currencyAmountField.setValue(getGoodsIncomeItem().getEnterCurrencyAmount());
        currencyTaxAmountField.setValue(getGoodsIncomeItem().getCurrencyTaxAmount());
   }


    protected void populateData() throws ApplicationException {
        getGoodsIncomeItem().setProduct((Product) goodsComboEdit.getValue());
        getGoodsIncomeItem().setQuantity(quantityField.floatValue());
        getGoodsIncomeItem().setUnit(unitComboBox.getSelectedUnit());
        getGoodsIncomeItem().setUnitRate(unitRateField.doubleValue());
        getGoodsIncomeItem().setEnterCurrencyPrice(currencyPriceField.doubleValue());
        getGoodsIncomeItem().setTaxId(taxPercentField.getTaxId());
        getGoodsIncomeItem().setEnterCurrencyAmount(currencyAmountField.doubleValue());
        getGoodsIncomeItem().setCurrencyTaxAmount(currencyTaxAmountField.doubleValue());

        //TODO: Calculate national amount
        getGoodsIncomeItem().calculateGeneral(getGoodsIncome().getCurrencyRate());
    }
