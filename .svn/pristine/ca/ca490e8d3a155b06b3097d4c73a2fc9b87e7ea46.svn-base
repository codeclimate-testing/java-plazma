...
public class SystemBrowser {
    ...
    /**
     * Display an URL in the system browser. If you want to display a file,
     * you must include the absolute path name.
     * 
     * @param url
     *                the document's url (the url must start with either
     *                "http://" or "file://").
     */
	...
	// in JavaWebStart.
	try {
	    Class serManClass = ClassUtilsExt
		    .forName("javax.jnlp.ServiceManager");
	    Class basSerClass = ClassUtilsExt
	...
	try {
	    ...
	    // OH: PlatformSystem.err.println("Trying to invoke browser,
                // cmd='" + connectStringArray(cmdLine) + "' ... ");
    ...
    // Try to invoke the browser specified in the BROWSER environment
	    ...
	    { "konqueror", "-remote", "openURL(" + URLTOKEN + ")" },

	    // The second guess for a browser under other systems (and
                // unix):
