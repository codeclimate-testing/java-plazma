...
public class NumberFormatter extends AbstractTextFormatter {
	...
	return NumberUtilities.convertNumber(value, valueClass);
   }
    ...
    protected void setPatterns(String edit, String display, Locale loc) {
      ...
      // Création du formatteur pour l'affichage
      nfDisplay = display != null ? new DecimalFormat(display) : nfEdit;

      // Initialisation du cache de saisie
      intCount = 0;
    ...
    public void verifyText(VerifyEvent e) {
      ...
      if ( e.keyCode == SWT.BS || e.keyCode == SWT.DEL ) {
      	...
      	for (int i = 0; i < e.text.length(); i++) {
      		...
      		if ( c >= '0' && c <= '9' ) {
      			// Contrôle du nombre de digits / zône
      			if ( d >= 0 && p > d ) {
