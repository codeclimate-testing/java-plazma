 ...
 * 
 * @author Oleh Hapon
 * $Id: CarServOrderItemPartEditFormRP.java,v 1.4 2010/04/28 06:23:01 ohapon Exp $
 */
public class CarServOrderItemPartEditFormRP extends AbstractGoodsDocumentItemEditForm {
    ...
    private XTextField supplierField;
    private XTextField docNumberField;
    private XTextField responsibleField;    
    private XTextArea noteField;
    
    

    public CarServOrderItemPartEditFormRP(Composite parent, int style) {
	super(parent, style);
	initialize();
    }
	...
	supplierLabel.setText(Messages.getString("CarServOrderItemEditForm.supplierLabel.text")); //$NON-NLS-1$

	supplierField = new XTextField(this, SWT.BORDER);
	gridData = new GridData(SWT.LEFT, SWT.CENTER, false, false, 3, 1);
	gridData.widthHint = 300;
	...
	docNumberLabel = new Label(this, SWT.NONE);
	docNumberLabel.setText(Messages.getString("CarServOrderItemEditForm.docNumberLabel.text")); //$NON-NLS-1$
	  
	docNumberField = new XTextField(this, SWT.BORDER); 
	gridData = new GridData(SWT.FILL, SWT.CENTER, false, false);
	gridData.widthHint = 100; 
	docNumberField.setLayoutData(gridData);

	label = new Label(this, SWT.NONE);
	label.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 2, 1));
	

	responsibleLabel = new Label(this, SWT.NONE);
	responsibleLabel.setText(Messages.getString("CarServOrderItemEditForm.responsibleLabel.text")); //$NON-NLS-1$

	responsibleField = new XTextField(this, SWT.BORDER);
	gridData = new GridData(SWT.LEFT, SWT.CENTER, false, false, 3, 1);
	gridData.widthHint = 300;
	...
	initUnitField(unitField);
    	...
    	initNoEditableField(supplierField);
    	initNoEditableField(docNumberField);
    	initNoEditableField(responsibleField);
	...
	bindControl(supplierField, "supplierName");
	bindControl(docNumberField, "documentTitle");
	bindControl(responsibleField, "responsibleName");
