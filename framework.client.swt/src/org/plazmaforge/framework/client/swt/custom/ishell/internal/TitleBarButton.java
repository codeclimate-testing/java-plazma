...
{
  private final Color highlightShadowColor, lightShadowColor, normalShadowColor, darkShadowColor;
  private final Color gradEndColor, inactiveGradEndColor, widgetBackgroundColor, widgetForegroundColor;
  ...
  public TitleBarButton(InternalShell parent, int style)
  {
    ...
    lightShadowColor = display.getSystemColor(SWT.COLOR_WIDGET_LIGHT_SHADOW);
    ...
    gradEndColor = display.getSystemColor(SWT.COLOR_TITLE_BACKGROUND_GRADIENT);
    inactiveGradEndColor = display.getSystemColor(SWT.COLOR_TITLE_INACTIVE_BACKGROUND_GRADIENT);
    widgetBackgroundColor = display.getSystemColor(SWT.COLOR_WIDGET_BACKGROUND);
    widgetForegroundColor = display.getSystemColor(SWT.COLOR_WIDGET_FOREGROUND);
    
    if((style & (SWT.CLOSE | SWT.MAX)) != 0) rightOff = 2;
    else leftOff = 2;
