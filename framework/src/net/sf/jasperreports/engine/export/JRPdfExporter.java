 ...
 * @version $Id: JRPdfExporter.java,v 1.2 2010/11/24 12:37:45 ohapon Exp $
 */
public class JRPdfExporter extends JRAbstractExporter
{
	...
	protected Font getFont(Map attributes, Locale locale, boolean setFontLines)
	{
		...
		else
		{
			...
			// FIX-START oha: 2010-11-24
			boolean loadPdfFont = false;
			if (fontInfo == null)
			{
				fontInfo = JRFontUtil.getFontInfo(jrFont.getPdfFontName(), locale);
				loadPdfFont = true;
			}
			// FIX-END
...
//					}
				...
				if (pdfFontName == null)
				{
				    ...
				    // FIX-START oha: 2010-11-24
				    if (!loadPdfFont && (face == null || face.getFile() == null))
				    {
					// 
					fontInfo = JRFontUtil.getFontInfo(jrFont.getPdfFontName(), locale);
					...
					{
					    //fontName found in font extensions
					    family = fontInfo.getFontFamily();
					    face = fontInfo.getFontFace();
